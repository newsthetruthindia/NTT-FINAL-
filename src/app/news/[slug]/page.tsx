...
export default async function NewsDetails({ 
  ...
  try {
    ...
    const renderAttributionLink = (name: string) => {
        if (isCitizen) {
            return (
                <Link href="/category/citizen-journalism" className="hover:text-primary transition-colors no-underline">
                    BY {name}
                </Link>
            );
        }
        ...
        return (
            <Link href={`/reporter/${post.user?.id || 1}`} className="hover:text-primary transition-colors no-underline tracking-[0.05em]">
                BY {name}
            </Link>
        );
    };
      ...
      <main className="min-h-screen bg-background text-foreground transition-colors duration-500">
        ...
        <article className="pt-20 pb-16 max-w-4xl mx-auto px-4 relative z-10 font-[family-name:var(--font-inter)]">
            ...
            <div className="flex flex-wrap items-center gap-6 mb-8 pb-6 border-b border-border/50 text-[11px] font-bold uppercase tracking-widest text-secondary">
                ...
                <div className="flex items-center gap-4 text-secondary/70 tracking-widest text-[10px]">
