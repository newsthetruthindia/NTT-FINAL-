...
const SocialSidebar: React.FC<SocialSidebarProps> = ({ title, url }) => {
  ...
  return (
    <div className={`fixed left-8 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-4 transition-all duration-700 hidden xl:flex ${visible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10 pointer-events-none'}`}>
      <div className="flex flex-col gap-3 p-3 bg-card/80 backdrop-blur-xl border border-border/50 rounded-full shadow-2xl overflow-hidden group">
        {shareLinks.map((link) => (
          <a
            key={link.name}
            href={link.href}
